...
### 絶対禁止：not_doneステートは不要

検査値変量に `not_done` ステートを含めてはならない。未観測変量は推理エンジンが自動的に辺縁化(marginalization)で処理するため、`not_done`は冗余であり、CPTの弁別力を希釈する。

### 絶対禁止：CPTの機械的一括変換

CPT（leak/parent_effects）を固定ロジックのプログラムで一括変換してはならない（例: 比例正規化、LR保存式など）。各CPTは臨床的根拠に基づいて**手動で**設計する必要がある。既存CPTの変更にはPMC/教科書の裏付けが必要（鉄則2）。
